...
class AddCategoryViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    // 계절 선택 프로퍼티
    private var selectedSeason: UIImageView?
    // 카테고리/계절 데이터 델리게이트
    ...
    override func viewDidLoad() {
        ...
        addCategoryView.toggleContainerView(hidden: false)
    ...
    // 계절 선택 이벤트
    private func setupSeasonTapGestures() {
       // 각 계절 이미지뷰에 탭 제스처 추가
       let springTap = UITapGestureRecognizer(target: self, action: #selector(seasonImageTapped(_:)))
       let summerTap = UITapGestureRecognizer(target: self, action: #selector(seasonImageTapped(_:)))
       let autumnTap = UITapGestureRecognizer(target: self, action: #selector(seasonImageTapped(_:)))
       let winterTap = UITapGestureRecognizer(target: self, action: #selector(seasonImageTapped(_:)))
       
       addCategoryView.springImageView.addGestureRecognizer(springTap)
       addCategoryView.summerImageView.addGestureRecognizer(summerTap)
       addCategoryView.autumnImageView.addGestureRecognizer(autumnTap)
       addCategoryView.winterImageView.addGestureRecognizer(winterTap)
   }
...
//        
        // 계절
        let selectedSeasonValue: String? = if let selectedSeason = selectedSeason {
            switch selectedSeason {
            case addCategoryView.springImageView: "SPRING"
            case addCategoryView.summerImageView: "SUMMER"
            case addCategoryView.autumnImageView: "FALL"
            case addCategoryView.winterImageView: "WINTER"
            default: nil
            }
        } else {
            nil
...
//        navigationController?.pushViewController(weatherVC, animated: true)
    ...
    @objc private func seasonImageTapped(_ gesture: UITapGestureRecognizer) {
       ...
       // 이미 선택된 이미지를 다시 탭하면 선택 해제
       if selectedSeason == tappedImageView {
           selectedSeason = nil
           addCategoryView.updateSelectedSeason(UIImageView()) // 빈 이미지뷰를 전달하여 모든 테두리 제거
       } else {
           selectedSeason = tappedImageView
           addCategoryView.updateSelectedSeason(tappedImageView)
       }
   }
